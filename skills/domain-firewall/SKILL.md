...
3. **Include subdomains explicitly** — `stripe.com` and `docs.stripe.com` are separate domains. List both, or use the code API with `pattern(["*.stripe.com"], "allow")` for glob matching.
